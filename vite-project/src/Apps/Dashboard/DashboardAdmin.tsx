...
const Dashboard = () => {
    ...
    useEffect(() => {
        const getAllUserNodos = async () => {
            const response = await axios.get(`${ManageNodosService.baseUrl}${ManageNodosService.endpoints.admin}/${user2}`);
        ...
        <Box className="flex flex-row">
            <Container disableGutters className="bg-green-200 flex flex-col justify-center rounded-lg items-center max-w-3x1 pl-5 pr-5 ">
                <Box
                    ...
                    <Grid item>
                        <Typography variant="h3">Totalidad de cultivos</Typography>
                    </Grid>
                </Box>
                <Grid container
                    direction="row"
